...
<body>
  ...
  <script>
    ...
    const authBox = document.getElementById('changePasswordForm');

    // Show loading animation on form submit
    form.addEventListener('submit', function(e) {
      // Hide the auth box completely
      if (authBox) {
        authBox.style.opacity = '0';
        authBox.style.pointerEvents = 'none';
        authBox.style.visibility = 'hidden';
      }
      // Show only the global loading overlay
      document.getElementById('globalLoadingOverlay').classList.add('active');
      // Hide the inner loading overlay (if visible)
      loadingOverlay.classList.remove('active');
      // Keep the loading overlay for at least 3 seconds
      setTimeout(function() {
